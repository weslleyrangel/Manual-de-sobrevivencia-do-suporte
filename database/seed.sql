-- Script de Seed do Banco de Dados PostgreSQL
-- 4 Usuários (1 Principal + 3 Técnicos) e 33+ Publicações Reais de TI

-- Limpeza prévia para reexecução idempotente
TRUNCATE TABLE solutions, problems, users RESTART IDENTITY CASCADE;
...
-- 2. Inserção das 20 Publicações do Usuário Principal (Weslley Rangel - ID 1)
...
-- 3. Inserção das Publicações de Outros Usuários (Ana, Rafael, Mariana)
INSERT INTO problems (id, author_id, title, category, description, status, views_count, likes_count, is_draft) VALUES
...
-- 4. Inserção das Soluções Passo a Passo para os Problemas
...
-- 5. Soluções Alternativas / Colaborativas (Requisito PRD de múltiplos métodos para um problema)
INSERT INTO solutions (problem_id, author_id, content, steps, is_primary) VALUES
